...
int main(){
  ...
  const float u_=0.2;     // Friction velocity
  const float k=0.41;     // Von Karman's constant
  const float z_zero=1.0; // Roughness length
  
  /* Arrays to store variables. These have NX+2 elements
     to alow boundary values to be stored at both ends */
  ...
  /* Array to store vertically averaged distrabution of u(x,y) */
  float avg[NX+2];
  ...
  /* LOOP 4 */
  #pragma parallel for default(none) shared(initialfile, x, y, u) collapse(2)
    for (int i=0; i<NX+2; i++){
      for (int j=0; j<NY+2; j++){
  ...
  /* LOOP 5 */
  /* This loop cannot be parallelized as it has a data dependency between iterations.
     Specifically, the computation of the dudt array in one iteration depends on the
     value of u from the previous iteration, so the iterations must be executed in 
  ...
  /* LOOP 10 */
  #pragma parallel for default(none) shared(finalfile, x, y, u)
  for (int i=0; i<NX+2; i++){
    for (int j=0; j<NY+2; j++){
  ...
  /*** Write array of average u values out to file ***/
  FILE *averagefile;
